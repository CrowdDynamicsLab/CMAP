...
def load_alpha_beta_k(alpha_k_path, beta_k_path):
	ALPHA_K = []
...
def load_group_topic_distr(group_topic_distr_path):
	GROUP_TOPIC = []

	group_topic_distr_file = codecs.open(group_topic_distr_path, 'r', 'utf-8')
...
def load_user_group_topic(table_assign_path, discount):
	...
	for row in table_assign_file:
		...
		PY_TERM[group] += (int(s[0]) - discount)
	...
	return USER_GROUP, USER_TABLE, PY_TERM
...
def compute_posteriors(INTR_GROUP, USER_MAP, WORD_IDX, BEHAV_IDX, PHI_W, PHI_B, GROUP_USER, ALPHA_G, BETA_G, ALPHA_K, BETA_K, GROUP_PRIOR, GROUP_TOPIC_PRIOR, POST_IDs, LINKS_i_j, LINK_PROB, USER_TABLE, PY_TERM, TIME_MAP, intr_path):
		...
		for g in range(G):
			...
			for k in range(K):
				...
				for word in text_li:
					...
					prob_k = prob_k * PHI_W[k][w]

				prob_k = prob_k * compute_time_prob(ALPHA_G[g][k], BETA_G[g][k], ts)

				prob_k *= GROUP_TOPIC_PRIOR[g][k]
	...
	for row in intr_file:
		...
		for g in range(G):
			for k in range(K):
				...
				curr_posterior_g_k.append(prob)
		...
		for g in range(G):
			prob_g = 0.0
			for k in range(K):
				prob_g += curr_posterior_g_k[li_idx]

				li_idx+=1

			curr_posterior_g.append(prob_g)

		curr_posterior_k = [0.0]*K

		li_idx = 0
		for g in range(G):
			for k in range(K):
				curr_posterior_k[k] += curr_posterior_g_k[li_idx]
				li_idx+=1
...
def generate_posteriors(basepath, intr_path, links_path, discount):
	...
	# basepath = "../Output/" +model+"_"+str(K)+"_"+str(G)+"_"+dataset+"_"+str(discount)+"00000/"
	...
	group_topic_distr_path = basepath+"group-topic-distribution.txt"
	...
	# GROUP_TOPIC_PRIOR = load_group_topic_distr(group_topic_distr_path)

	USER_GROUP, USER_TABLE, PY_TERM = load_user_group_topic(table_assign_path, discount)
	...
	DICT_USER_POSTERIORS = compute_posteriors(USER_GROUP, USER_MAP, WORD_IDX, BEHAV_IDX, PHI_W, PHI_B, GROUP_USER, ALPHA_G, BETA_G, ALPHA_K, BETA_K, GROUP_PRIOR, GROUP_TOPIC_PRIOR, POST_IDs, LINKS_i_j, LINK_PROB, USER_TABLE, PY_TERM, TIME_MAP, intr_path)
	...
	# print(DICT_USER_POSTERIORS, file = posterior_file)
...
parser = argparse.ArgumentParser("Posterior_Unified")
...
parser.add_argument("--K", help="Number of Topics", default = "5")
