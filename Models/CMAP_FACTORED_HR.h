...
class CMAP_FACTORED_HR{

public:
	string model_type = "CMAP_FACTORED_HR";
	...
	// map<pair<int, int>, vector<int>> g_k_tables;
	...
	// vector<vector<double>> pkt;	// Beta Distribution probability of the topic k at time t (K x RESOLUTION)
...
public:
	...
	CMAP_FACTORED_HR(int _G = 20, int _K_b = 5, int _K_w = 20, double _discount = 0.5, double _scale = 1.5)
	{
		this->G = _G;
		this->K_b = _K_b;
		this->K_w = _K_w;
	...
	void seat_to(int itr, int u, int table, vector<vector<double>> p_k_w)
	{
	...
	double compute_p_u_k_b_hr(int itr, int u, int k_b)
	{
		double p = 0.0;
		...
		for(int b = 0; b < behav_cnts.size(); b++)
		{
			...
			p = p + (USER_BEHAV_CNT[u][b]) * log( ((B_TOPIC_BEHAV_INTR_CNT[k_b][b] - ((B_TOPIC_BEHAV_NUM_TABLES[k_b][b])*(discount))) / (B_TOPIC_TOTAL_INTR_CNT[k_b] +scale))  +  (((scale + B_TOPIC_NUM_TABLES[k_b])/(B_TOPIC_TOTAL_INTR_CNT[k_b] + scale)) * (1.0/B))) ;
		}
		...
		for(int u = 0; u < U; ++u)
		{
			...
			for(int k_b = 0; k_b < K_b; ++k_b)
			{
				...
				for(int b = 0; b < behav_cnts.size(); b++)
				{
					...
					b_term[k_b] += (USER_BEHAV_CNT[u][b]) * log( ((B_TOPIC_BEHAV_INTR_CNT[k_b][b] - ((B_TOPIC_BEHAV_NUM_TABLES[k_b][b])*(discount))) / (B_TOPIC_TOTAL_INTR_CNT[k_b] +scale))  +  (((scale + B_TOPIC_NUM_TABLES[k_b])/(B_TOPIC_TOTAL_INTR_CNT[k_b] + scale)) * (1.0/B))) ;
				}
			}
			...
			seat_to(itr, u, sampled_table, p_k_w);			// Update Table assignment and associated count matrices and arrays
	...
	void output_result(string destpath)
	{
		...
		for(int g = 0; g < G; ++g)
		{
			...
			// double prob = (ng[g] + alpha_gt) / (C + G * (alpha_gt));
		...
		for(int g = 0; g < G; ++g)
		{
			for(int k_b = 0; k_b < K_b; ++k_b)
			{
				...
				// double prob = ((ngk[g][k] + alpha_gtk) / (ng[g] + K * (alpha_gtk)));
		...
		for(int k_b = 0; k_b < K_b; ++k_b)
		{
			for(int b = 0; b < B; ++b)
			{
				double prob = ( ((B_TOPIC_BEHAV_INTR_CNT[k_b][b] - (B_TOPIC_BEHAV_NUM_TABLES[k_b][b] * discount)) / (B_TOPIC_TOTAL_INTR_CNT[k_b] + scale)) + (((scale + B_TOPIC_NUM_TABLES[k_b])/(B_TOPIC_TOTAL_INTR_CNT[k_b] + scale)) * (1.0/B)));
		...
		for (int k_b = 0; k_b < K_b; k_b++) {
			...
			for (int b = 0; b < B; b++) {
				word_prob.first = b;
				word_prob.second = ( ((B_TOPIC_BEHAV_INTR_CNT[k_b][b] - (B_TOPIC_BEHAV_NUM_TABLES[k_b][b] * discount)) / (B_TOPIC_TOTAL_INTR_CNT[k_b] + scale)) + (((scale + B_TOPIC_NUM_TABLES[k_b])/(B_TOPIC_TOTAL_INTR_CNT[k_b] + scale)) * (1.0/B)));
				words_probs.push_back(word_prob);
			}
