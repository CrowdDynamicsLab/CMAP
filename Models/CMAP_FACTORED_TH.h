...
class CMAP_FACTORED_TH{

	typedef  void* (CMAP_FACTORED_TH::*CMAP_FACTORED_THPtr)(void *);
	typedef  void* (*PthreadPtr)(void*);


public:
	string model_type = "CMAP_FACTORED_TH";
...
public:
	CMAP_FACTORED_TH(int _G = 20, int _K_b = 5, int _K_w = 20, double _discount = 0.5, double _scale = 1.5, int _NUM_TH = 16)
	{
		this->G = _G;
		this->K_b = _K_b;
		this->K_w = _K_w;
...
// 				prob_distr_g_k[idx] += links_prob[g];
				...
				// if(itr != 0)
				// {
				// 	cout<<"g: "<<g<<", k: "<<k_b<<", p: "<<prob_distr_g_k[idx]<<", "<<b_term[k_b]<<", "<<word_topic_term[g]<<", "<<log(g_k_const[g][k_b] + (((scale + (T - empty_tables.size())*discount) / (N - 1 + scale)) * exp(GROUP_PRIOR[g]) * exp(GROUP_G_B_PRIOR[g][k_b]) ))<<", "<<ts_term[g]<<endl;
				// }

				// else
				// {
				// 	cout<<"g: "<<g<<", k: "<<k_b<<", p: "<<prob_distr_g_k[idx]<<", "<<b_term[k_b]<<", "<<word_topic_term[g]<<", "<<log((g_k_const_num[g][k_b] / (N - 1 + scale)) + (((scale + (T - empty_tables.size())*discount) / (N - 1 + scale)) * exp(GROUP_PRIOR[g]) * exp(GROUP_G_B_PRIOR[g][k_b]) ))<<", "<<ts_term[g]<<endl;
				// }
		...
		// num_interactions = user_intr_list[u].size();
		...
		// returned_struct->sampled_table = sampled_table;
		returned_struct->sampled_group = sampled_group;
		returned_struct->sampled_b_topic = sampled_b_topic;
		// returned_struct->new_table = new_table;	
		// returned_struct->p_k_w = p_k_w;
		...
		else
		{
			...
			for(int u = 0; u < U; u+=NUM_THREADS)
			{
				...
				for(long uu = u; uu < u+NUM_THREADS && uu < U; uu++)
				{
					...
					CMAP_FACTORED_THPtr t11 = &CMAP_FACTORED_TH::sample_table;
				...
				// Create Threads for Sampling word topics
