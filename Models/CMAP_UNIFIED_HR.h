...
class CMAP_UNIFIED_HR{

public:
	string model_type = "CMAP_UNIFIED_HR";
...
public:
	...
	CMAP_UNIFIED_HR(int _G = 20, int _K = 20, int _discount = 0.5, int _scale = 1.5)
	{
		this->G = _G;
		this->K = _K;
	...
	double compute_p_i_k_hr(int itr, int i, int k)
	{
		double p = 0.0;
		...
		p = p + log( ((B_TOPIC_BEHAV_INTR_CNT[k][b] - ((B_TOPIC_BEHAV_NUM_TABLES[k][b])*(discount))) / (B_TOPIC_TOTAL_INTR_CNT[k] +scale))  +  (((scale + B_TOPIC_NUM_TABLES[k])/(B_TOPIC_TOTAL_INTR_CNT[k] + scale)) * (1.0/B)));
	...
	void seat_to(int itr, int u, int table, vector<vector<double>> p_k)
	{
		...
		for(int i = 0; i < num_interactions; i++)
		{
			...
			int k_b = intr_topic[user_intr_list[u][i]];
	...
	void gibbs_sampling_iteration(int itr, clock_t last_time, string destpath)
	{
		...
		for(int u = 0; u < U; ++u)
		{
			...
			for(int i = 0; i < num_interactions; i++)
			{
				...
				for(int k = 0; k < K; ++k)
				{
					p_k[i][k] = compute_p_i_k_hr(itr, user_intr_list[u][i], k);
					curr_p_k[k] = p_k[i][k];
				}
	...
	void initialize(const char *corpus_path,const char *link_path)
	{
		...
		B_TOPIC_NUM_TABLES.resize(K, 0);
		B_TOPIC_TABLE_INTR_CNT.resize(K);
		B_TOPIC_TOTAL_INTR_CNT.resize(K, 0);
		B_TOPIC_EMPTY_TABLES.resize(K);

		B_TOPIC_BEHAV_NUM_TABLES.resize(K);
		B_TOPIC_BEHAV_TABLE_LIST.resize(K);
		B_TOPIC_BEHAV_INTR_CNT.resize(K);

		for(int k = 0; k < K; k++)
		{
			B_TOPIC_BEHAV_NUM_TABLES[k].resize(B, 0);
			B_TOPIC_BEHAV_TABLE_LIST[k].resize(B);
			B_TOPIC_BEHAV_INTR_CNT[k].resize(B, 0);
		}
	...
	void output_result(string destpath)
	{
		...
		for(int k = 0; k < K; ++k)
		{
			for(int b = 0; b < B; ++b)
			{
				double prob = ( ((B_TOPIC_BEHAV_INTR_CNT[k][b] - (B_TOPIC_BEHAV_NUM_TABLES[k][b] * discount)) / (B_TOPIC_TOTAL_INTR_CNT[k] + scale)) + (((scale + B_TOPIC_NUM_TABLES[k])/(B_TOPIC_TOTAL_INTR_CNT[k] + scale)) * (1.0/B)));
		...
		for (int k = 0; k < K; k++) {
			...
			for (int b = 0; b < B; b++) {
				word_prob.first = b;
				word_prob.second = ( ((B_TOPIC_BEHAV_INTR_CNT[k][b] - (B_TOPIC_BEHAV_NUM_TABLES[k][b] * discount)) / (B_TOPIC_TOTAL_INTR_CNT[k] + scale)) + (((scale + B_TOPIC_NUM_TABLES[k])/(B_TOPIC_TOTAL_INTR_CNT[k] + scale)) * (1.0/B)));
				words_probs.push_back(word_prob);
			}
...
// 	string destpath = "../../"+model_type+"_"+to_string(K)+"_"+to_string(G)+"_"+dataset+"/";
